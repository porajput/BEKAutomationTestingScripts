...
public class Utils {
	 ...
	 * 
	 */
	public boolean MouseHoverAndClick(WebElement elem) throws Exception {
		boolean flag = false;
		try {
			Actions act = new Actions(LocalDriverManager.getDriver());
	...
	public boolean elementExistsList(List<WebElement> elem){
	    return !elem.isEmpty();
	}
	...
	public void selectByTextInList(List<WebElement> element, String string) throws Throwable {
		...
		try {
			...
			{
				if(element.get(i).getText().trim().contains(string))
				{
	...
	/**
	 * @param elementLocator - element on which action is being performed
	 * @return - webElement which is being highlighted
	 */
	public WebElement highLight(WebElement elementLocator){
		JavascriptExecutor js;
		try {
			js = (JavascriptExecutor) LocalDriverManager.getDriver();
			js.executeScript("arguments[0].setAttribute('style', arguments[1]);", elementLocator, " border: 3px solid purple;");
			Thread.sleep(50);
		} catch (Exception e) {
			ExecutionHelper.getLogger().log(LogStatus.FAIL,"Failed to highlight element on which action is being performed: " + e.getMessage());
			elementLocator = null;
		}
